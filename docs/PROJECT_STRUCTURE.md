...
│
├── scripts/
│   ├── build_index.py                   # CLI: rebuild FAISS vector index from database
│   └── validate_candidates.py           # CLI: validate sample_candidates.csv structure
│
├── tests/
│   ├── __init__.py
...
**Purpose:** Standalone command-line scripts for database and index maintenance.

**Business responsibility:** Provides one-off maintenance tasks that are run manually by developers, not as part of the application runtime.

| File | Responsibility |
|------|----------------|
| `build_index.py` | Rebuild the FAISS vector index from all candidates in the database |
| `validate_candidates.py` | Validate `data/sample_candidates.csv` for correct structure and data integrity |

**Dependencies:** `database/crud`, `database/db_setup`, `embeddings/embedder`, `embeddings/vector_store`.

**Used by:** Developers running manual maintenance commands.

**Pipeline fit:** These scripts are not part of the application runtime. They are invoked manually during setup or when sample data changes.
...
| `scripts/` | CLI maintenance scripts — FAISS index rebuild, CSV data validation |
...
### `scripts/validate_candidates.py` — CSV Validation

**Command:** `python scripts/validate_candidates.py`
...
| 9 | Root files match the repository | ✅ All 7 root files verified: `.env`, `.gitignore`, `INSTALL.md`, `LICENSE`, `README.md`, `render.yaml`, `requirements.txt` |
